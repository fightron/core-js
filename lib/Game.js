...
import testPose from '../test/fixtures/poses/test-pose'
import testPoseLow from '../test/fixtures/poses/test-pose-low'
import testPoseUp from '../test/fixtures/poses/test-pose-up'
...
export class Game extends Base {
  ...
  demo () {
    ...
    this.sendToClient('+', 'po', testPose)
    this.sendToClient('+', 'po', testPoseLow)
    this.sendToClient('+', 'po', testPoseUp)
    this.sendToClient('+', 'am', testAnimation)

    this.sendToClient('po', '1-1', 'test-pose')
    this.sendToClient('po', '1-2', 'test-pose-up')
    setTimeout(() => {
      this.sendToClient('am', '1-1', 'test-animation')
    }, 1000)
    setTimeout(() => {
      this.sendToClient('am', '1-2', 'test-animation')
    }, 1200)

    // this.sendToClient('po', '2-1', 'test-3-l-0')
    // this.sendToClient('po', '2-2', 'test-3')
